...
class FilterRepository:
    ...
    def check_email(self, email: str) -> Optional[str]:
        ...
        for filter_item in filters:
            ...
            # ... (target selection - IMPROVED to handle all localpart patterns)
            cat_lower = category.lower()
            # Localpart patterns: prefix, localpart, uuid, md5, hash, bot, desk, workday, tracking, digit, random, reply, plus
            if any(k in cat_lower for k in ['localpart', 'prefix', 'uuid', 'md5', 'hash', 'bot', 'desk', 'workday', 'tracking', 'digit', 'random', 'reply', 'plus']):
                match_target = local_part
            elif 'domain' in cat_lower or 'tld' in cat_lower or 'subdomain' in cat_lower:
                match_target = domain
    ...
    def _is_dynamic_junk(self, local_part: str, domain: str) -> bool:
        ...
        NOTE: Most patterns are now in keywords.csv (rows 19, 71-73, 76-82).
        This method only contains algorithmic heuristics that can't be expressed as simple regex patterns.
        """
        
        # ALGORITHMIC HEURISTIC: High digit density (5+ total digits in localpart)
        # This is different from CSV row 72 which checks for 4+ CONSECUTIVE digits
        # This checks for 5+ digits ANYWHERE in the localpart (e.g., "user1test2abc3xyz4@domain.com")
        ...
        if digits >= 5:
            ...
            if any(c.isalpha() for c in local_part):
                self.logger.info(f"Dynamic junk: High digit density ({digits} digits) detected in {local_part}")
                return True
